...
<CONTROL>
	...
	create/rewriteオペレーションでは、単にローカルのファイルパス以外に、
	HTTP-URIを指定することもできます。
	PATHが「http://」から始まっている場合は、それをHTTP-URIと見なしてダウンロードを試みます。
	extractオペレーションでは、出力するパスを指定します。
	CONTROLは単に引数のように指定します。

    CONTROL:
      -p PATH | HTTP-URI  PARAM.SFO    PBPメタデータファイル。
      -m PATH | HTTP-URI  ICON0.PNG    メインアイコンファイル。
      -a PATH | HTTP-URI  ICON1.PMF    アニメーションアイコンファイル。
      -f PATH | HTTP-URI  PIC0.PNG     フロート背景画像。
      -b PATH | HTTP-URI  PIC1.PNG     背景画像。
      -s PATH | HTTP-URI  SND0.AT3     BGMファイル。
